...
timeOverFileName = sprintf('%s/%s_timeover_%s.mat',baseFolder, objFctName, gethostname);
if exist('./data', 'dir')
...
if forceSaveFile || mbtime - nextSaveTime >= 0
  ...
  if isempty(resultFileName)
    ...
    fileName = sprintf('%s/%s_lastresultnumber.mat', baseFolder,objFctName);
    if exist('./data', 'dir')
    ...
    % build file name
    resultFileName = sprintf('%s/%s_result_%s_%02d.mat',baseFolder,objFctName, hostname, resultFileNr);
    if exist('./data', 'dir')
